        ...
        if self.iterables:
            ...
            try:
                ...
                self.collection.responses[rendered_content.name] = {
                    'data': {'error': '"{}" not found'.format(exception.args[0]['path'])},
                    'include': 'true',
                    'is_response': True,
                    'status_code': 404
                }
                ...
                environ = self.collection.view_request.environ.copy()
                ...
                request.LANGUAGE_CODE = self.collection.view_request.LANGUAGE_CODE
                request.POST = post_query_dict
                request._read_started = True
                request.auth = self.collection.view_request.auth
                request.csrf_processing_done = True
                request.session = self.collection.view_request.session
                request.user = self.collection.view_request.user
            ...
            if self.group_name:
                ...
                self.collection.context['groups'].setdefault(self.group_name, [])
