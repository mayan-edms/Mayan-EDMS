...
class DocumentFileAPIViewTestCase(
    ...
    def test_document_file_api_edit_via_patch_view_no_permission(self):
        self._upload_test_document()

        response = self._request_test_document_file_api_edit_via_patch_view()
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_document_file_api_edit_via_patch_view_with_access(self):
        self._upload_test_document()
        self.grant_access(
        ...
        response = self._request_test_document_file_api_edit_via_patch_view()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.test_document.latest_file.refresh_from_db()
        self.assertEqual(self.test_document.files.count(), 1)
        self.assertEqual(
            self.test_document.latest_file.comment,
            TEST_DOCUMENT_VERSION_COMMENT_EDITED
        )

    def test_document_file_api_edit_via_put_view_no_permission(self):
        self._upload_test_document()

        response = self._request_test_document_file_api_edit_via_put_view()
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_document_file_api_edit_via_put_view_with_access(self):
        self._upload_test_document()
        self.grant_access(
            obj=self.test_document, permission=permission_document_edit
        )
    ...
    def test_document_file_api_revert_view_no_permission(self):
        self._upload_test_document()
        self._upload_new_file()
    ...
    def test_document_file_api_revert_view_with_access(self):
        self._upload_test_document()
        self._upload_new_file()
        self.grant_access(
            obj=self.test_document,
