...
class KeyAPIViewTestMixin(object):
    def _request_test_key_create_api_view(self, extra_data=None):
        data = {
        ...
        return self.post(viewname='rest_api:key-list', data=data)

    def _request_test_key_destroy_api_view(self):
        return self.delete(
    ...
    def _request_test_key_list_api_view(self):
        return self.get(viewname='rest_api:key-list')

    def _request_test_key_retreive_api_view(self):
        return self.get(
            viewname='rest_api:key-detail', kwargs={
                'key_id': self.test_key_private.pk
            }
...
class KeyViewTestMixin(object):
    ...
    def _request_test_key_detail_view(self):
        return self.get(
            viewname='django_gpg:key_detail', kwargs={
                'key_id': self.test_key_private.pk
            }
        )

    def _request_test_key_private_list_view(self):
        return self.get(viewname='django_gpg:key_private_list')

    def _request_test_key_public_list_view(self):
        return self.get(viewname='django_gpg:key_public_list')
