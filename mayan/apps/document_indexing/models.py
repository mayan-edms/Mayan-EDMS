...
class IndexTemplate(ExtraDataModelMixin, models.Model):
        ...
        try:
            return reverse(
                viewname='indexing:index_instance_node_view', kwargs={
                    'index_instance_node_id': self.index_instance_root_node.pk
                }
            )
    ...
    def rebuild(self):
        ...
        if self.enabled:
            ...
            # deleted.
            try:
                self.index_instance_root_node.delete()
            except IndexInstanceNode.DoesNotExist:
...
class IndexInstance(IndexTemplate):
    ...
    def get_instance_node_count(self):
        try:
            return self.index_instance_root_node.get_descendant_count()
        except IndexInstanceNode.DoesNotExist:
            return 0

    def get_item_count(self, user):
        try:
            return self.index_instance_root_node.get_item_count(user=user)
        except IndexInstanceNode.DoesNotExist:
            return 0
