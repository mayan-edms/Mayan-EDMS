...
def condition_is_current_user(context, resolved_object):
    return context['request'].user == resolved_object and not condition_otp_is_enabled(context=context, resolved_object=resolved_object)


def condition_otp_is_enabled(context, resolved_object):
    return resolved_object.otp_data.is_enabled()


link_otp_detail = Link(
    icon=icon_otp_detail, text=_('OTP details'), view='authentication_otp:otp_detail'
)
